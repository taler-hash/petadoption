<template>
    <Dialog v-model:visible="visible" modal :header="`Add Shelter`" @hide="close">
...
async function submit() {
    form.post(route('shelters.store'), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Success', detail: `Added Shelter Successfully`, life: 3000 });
            close()
            form.reset()
        }
    })
